...
// GET /api/posts?mode=public|private&sort=desc|asc
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase
    .from("posts")
    .select("id, title, description, photos, date, created_at")
    .eq("is_private", false)
    .order("date", { ascending: sort === "asc" });
...
export async function POST(request: NextRequest) {
  ...
  const { title, description, photos, is_private, date } = body;
